...
public extension RJSLib {
    class SimpleNetworkAgent: SimpleNetworkClientProtocol {
        ...
        // MARK: - ApiClient
        public func execute<T>(request: SimpleNetworkClientRequestProtocol, completionHandler: @escaping (Result<RJSLibNetworkAgentResponse<T>>) -> Void) {
            ...
            if request.mockedData != nil {
                ...
                if mockedData.count>0 {
                    do {
                        ...
                        let response = try RJSLibNetworkAgentResponse<T>(data: data, httpUrlResponse: nil, responseType: request.responseType)
            ...
            let dataTask = urlSession.dataTask(with: request.urlRequest) { (data, response, error) in
                ...
                guard let httpUrlResponse = response as? HTTPURLResponse else {
                    completionHandler(.failure(RJSLibNetworkAgentErrorsManager.Custom.with(error: error!)))
                    return
                }
                ...
                let doWork = {
                    ...
                    if successRange.contains(httpUrlResponse.statusCode) {
                        do {
                            let response = try RJSLibNetworkAgentResponse<T>(data: data, httpUrlResponse: httpUrlResponse, responseType: request.responseType)
                        ...
                        if request.debugRequest {
                            RJSLibNetworkAgentErrorsManager.logError(response: httpUrlResponse, request: request)
                        }
                        completionHandler(.failure(RJSLibNetworkAgentErrorsManager.APIError(data: data, httpUrlResponse: httpUrlResponse)))
                    }
                }
